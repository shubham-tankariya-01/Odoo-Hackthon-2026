...
class BookingRepository:
    ...
    async def update_status(self, booking: ResourceBooking,
                            new_status: str) -> ResourceBooking:
        booking.status = new_status  # type: ignore
        await self.session.commit()
        await self.session.refresh(booking)
        return booking

    async def get_conflicting_bookings(self, asset_id: UUID, new_start, new_end,
                                       exclude_booking_id: Optional[UUID] = None) -> List[ResourceBooking]:
        stmt = select(ResourceBooking).where(
            and_(
                ResourceBooking.asset_id == asset_id,
                ResourceBooking.status.in_(["upcoming", "ongoing"]),
    ...
    async def get_by_asset_and_month(
            self, asset_id: UUID, year: int, month: int) -> List[ResourceBooking]:
