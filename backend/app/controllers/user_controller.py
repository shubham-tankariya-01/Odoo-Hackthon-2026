...
class UserController:
    @staticmethod
    async def get_all(db: AsyncSession, department_id: Optional[UUID] = None, role: Optional[str] = None, status: Optional[str] = None, page: int = 1, page_size: int = 20):
        ...
        items = [UserProfile.model_validate(u).model_dump(mode='json') for u in users]
    ...
    @staticmethod
    async def promote(user_id: UUID, data: UserRoleUpdate, db: AsyncSession, current_user_id: UUID):
        service = UserService(UserRepository(db), DepartmentRepository(db))
