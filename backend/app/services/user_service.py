...
class UserService:
    ...
    async def get_all(self, department_id: Optional[UUID] = None, role: Optional[str] = None, status: Optional[str] = None, skip: int = 0, limit: int = 100) -> Tuple[List[User], int]:
    ...
    async def promote(self, user_id: UUID, data: UserRoleUpdate, promoter_id: UUID) -> User:
        user = await self.user_repo.get_by_id(user_id)
        if not user:
            raise NotFoundException("User not found")
