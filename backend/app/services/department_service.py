...
class DepartmentService:
    ...
    # type: ignore
    async def get_all(self, status: str = None, skip: int = 0,
                      limit: int = 100) -> Tuple[List[dict], int]:
        # For simplicity, returning raw models directly in real app we might
    ...
    async def get_by_id(self, dept_id: UUID) -> dict:
        ...
        # type: ignore
        head_user = await self.user_repo.get_by_id(dept.head_user_id) if dept.head_user_id else None
        children = await self.dept_repo.get_children(dept.id)  # type: ignore
        # type: ignore
        member_count = await self.dept_repo.get_member_count(dept.id)
    ...
    async def update(self, dept_id: UUID,
                     data: DepartmentUpdate) -> Department:
        dept = await self.dept_repo.get_by_id(dept_id)
        if not dept:
            raise NotFoundException("Department not found")
        ...
        if data.status == DepartmentStatus.INACTIVE:
            # Check for active users/allocations - for hackathon, just active
            # users
            # type: ignore
            count = await self.dept_repo.get_member_count(dept.id)
            if count > 0:
                raise ConflictException(
                    f"Cannot deactivate department with {count} active users")
