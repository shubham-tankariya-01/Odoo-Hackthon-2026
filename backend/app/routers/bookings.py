...
async def get_booking_calendar(asset_id: UUID, month: str, db: AsyncSession = Depends(get_db)):
    return await BookingController.get_calendar(asset_id, month, db)

@router.get("/availability", response_model=AvailabilityResponse)
async def check_booking_availability(asset_id: UUID, start: str, end: str, db: AsyncSession = Depends(get_db)):
    return await BookingController.check_availability(asset_id, start, end, db)

@router.get("/{booking_id}", response_model=BookingResponse)
async def get_single_booking_route(booking_id: UUID, db: AsyncSession = Depends(get_db)):
    return await BookingController.get_booking(booking_id, db)

@router.patch("/{booking_id}", response_model=BookingResponse)
async def reschedule_booking_route(booking_id: UUID, booking_update: BookingUpdate, current_user = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
