...
async def list_users(
    ...
    current_user = Depends(require_role([UserRole.ADMIN, UserRole.DEPARTMENT_HEAD, UserRole.ASSET_MANAGER]))
):
...
async def get_user(
    user_id: UUID,
    db: AsyncSession = Depends(get_db),
    current_user = Depends(require_role([UserRole.ADMIN, UserRole.DEPARTMENT_HEAD]))
):
