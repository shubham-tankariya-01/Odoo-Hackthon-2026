...
async def create_audit_cycle(cycle_in: AuditCycleCreate, db: AsyncSession = Depends(get_db)):
...
async def update_audit_cycle(cycle_id: UUID, cycle_in: AuditCycleUpdate, db: AsyncSession = Depends(get_db)):
...
async def close_audit_cycle(cycle_id: UUID, db: AsyncSession = Depends(get_db)):
